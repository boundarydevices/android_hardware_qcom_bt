...
int wait_for_data(int fd, int maxTimeOut)
{
	...
	if (maxTimeOut <= 0) {
		ALOGE("%s: Invalid timeout value specified", __func__);
		return -EINVAL;
	}
	...
	if (select (fd + 1, &infids, NULL, NULL, &timeout) < 1) {
		ALOGE("%s: Timing out on select for %d secs.\n", __FUNCTION__, maxTimeOut);
		return -1;
	}
	else
		ALOGD("%s: HCI-VS-EVENT available in TTY Serial buffer\n",
		      __FUNCTION__);
...
int read_vs_hci_event(int fd, unsigned char* buf, int size)
{
	...
	ALOGD("%s: Wait for HCI-Vendor Specfic Event from SOC", __FUNCTION__);
	...
	while (1) {
		...
		if (buf[0] == 0x04) { /* Recvd. HCI DATA */
			...
			retry++;
			ALOGI("%s: Not an HCI-VS-Event! buf[0]: %d",
			      __FUNCTION__, buf[0]);
			if (wait_for_data(fd, SELECT_TIMEOUT) < 1)
				return -1;
			...
			ALOGE("%s: RETRY failed!", __FUNCTION__);
			return -1;
		}
	}
	count++;

	ALOGD("%s: Wait for HCI-Vendor Specfic Event from SOC, buf[0] - 0x%x\n", __FUNCTION__, buf[0]);
	...
	ALOGD("%s: Wait for HCI-Vendor Specfic Event from SOC, buf[1] - 0x%x\n", __FUNCTION__, buf[1]);
	...
	ALOGD("%s: Wait for HCI-Vendor Specfic Event from SOC, count - 0x%x\n", __FUNCTION__, count);
	return count;
}
...
int hci_send_vs_cmd(int fd, unsigned char *cmd, unsigned char *rsp, int size)
{
	...
	if (wait_vsc_evt) {
		...
		ALOGD("%s: Received HCI-Vendor Specific Event from SOC", __FUNCTION__);
	}
		       ...
		       unsigned int p_base_addr, int segtNo, int size)
{
	...
	switch (edl_cmd) {
		case EDL_PATCH_SET_REQ_CMD:
			...
			ALOGV("%s: Sending EDL_PATCH_SET_REQ_CMD", __FUNCTION__);
		...
		case EDL_PATCH_ATCH_REQ_CMD:
			ALOGV("%s: Sending EDL_PATCH_ATTACH_REQ_CMD", __FUNCTION__);
			ALOGV("HCI-CMD %d:\t0x%x \t0x%x \t0x%x \t0x%x \t0x%x",
			      segtNo, cmd[0], cmd[1], cmd[2], cmd[3], cmd[4]);
			break;
		case EDL_PATCH_RST_REQ_CMD:
			ALOGV("%s: Sending EDL_PATCH_RESET_REQ_CMD", __FUNCTION__);
			ALOGV("HCI-CMD %d:\t0x%x \t0x%x \t0x%x \t0x%x \t0x%x",
			      segtNo, cmd[0], cmd[1], cmd[2], cmd[3], cmd[4]);
			break;
		case EDL_PATCH_VER_REQ_CMD:
			ALOGV("%s: Sending EDL_PATCH_VER_REQ_CMD", __FUNCTION__);
			ALOGV("HCI-CMD %d:\t0x%x \t0x%x \t0x%x \t0x%x \t0x%x",
			      segtNo, cmd[0], cmd[1], cmd[2], cmd[3], cmd[4]);
			break;
	...
	if (gTlv_type == TLV_TYPE_PATCH) {
		...
		switch (gTlv_dwndCfg)
		{
			case ROME_SKIP_EVT_NONE:
				wait_vsc_evt = TRUE;
				wait_cc_evt = TRUE;
				ALOGV("%s: Event handling type: ROME_SKIP_EVT_NONE", __func__);
				break;
			case ROME_SKIP_EVT_VSE_CC:
				wait_vsc_evt = FALSE;
				wait_cc_evt = FALSE;
				ALOGV("%s: Event handling type: ROME_SKIP_EVT_VSE_CC", __func__);
				break;
				/* Not handled for now */
			case ROME_SKIP_EVT_VSE:
			case ROME_SKIP_EVT_CC:
			default:
				ALOGE("%s: Unsupported Event handling: %d", __func__, gTlv_dwndCfg);
				break;
		}
	} else {
		wait_vsc_evt = TRUE;
...
static void flow_control(int fd, int opt)
{
	...
	if (opt == MSM_ENABLE_FLOW_CTRL)
		...
		ALOGE("%s: Incorrect option passed for TIOCMSET\n", __func__);
		return;
	}
...
int rome_set_baudrate_req(int fd, unsigned char baud_rate)
{
	...
	if ((err =read_vs_hci_event(fd, rsp, HCI_MAX_EVENT_SIZE)) < 0) {
		ALOGE("%s: Failed to get HCI-VS Event from SOC\n", __FUNCTION__);
		goto error;
	}

	ALOGD("%s: Received HCI-Vendor Specific Event from SOC\n", __FUNCTION__);

	/* Wait for command complete event */
	err = read_hci_event(fd, rsp, HCI_MAX_EVENT_SIZE);
	if ( err < 0) {
		ALOGE("%s: Failed to set patch info on Controller\n", __FUNCTION__);
		goto error;
	}
	ALOGD("%s\n", __FUNCTION__);
error:
...
int rome_soc_init(int fd, int speed)
{
	int err = -1;
	int ret = 0;
	int size;
	...
	if ((err = rome_patch_ver_req(fd)) < 0) {
		ALOGE("%s: Fail to get Rome Version (0x%x)\n", __FUNCTION__, err);
		ret = -1;
		goto error;
	}

	ALOGD("%s: Rome Version (0x%08x)\n", __FUNCTION__, rome_ver);

	switch (rome_ver){
		case ROME_VER_1_0:
			/* Set and Download the RAMPATCH */
			ALOGD("%s: Setting Patch Header & Downloading Patches\n", __FUNCTION__);
			err = rome_download_rampatch(fd);
			if (err < 0) {
				ALOGE("%s: DOWNLOAD RAMPATCH failed!\n", __FUNCTION__);
				ret = -1;
				goto error;
			}

			/* Attach the RAMPATCH */
			ALOGE("%s: Attaching the patches\n", __FUNCTION__);
			err = rome_attach_rampatch(fd);
			if (err < 0) {
				ALOGE("%s: ATTACH RAMPATCH failed!\n", __FUNCTION__);
				ret = -1;
				goto error;
			}
...
download:
			...
			err = rome_set_baudrate_req(fd, BAUDRATE_1000000);
			if (err < 0) {
		...
		default:
			ALOGD("%s: Detected unknown ROME version\n", __FUNCTION__);
			ret = -1;
			break;
	}
